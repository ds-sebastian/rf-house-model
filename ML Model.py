...
try:
    ...
    X_test = pd.DataFrame.sparse.from_spmatrix(
        X_test, columns=get_ct_feature_names(preprocessor))
...
# %%
model = XGBRegressor(**reg_best_params, tree_method='gpu_hist', gpu_id=0, random_state=42
                     )
...
if hyper_param_tune == True:
    ...
    model = BayesSearchCV(model, param_dist, verbose=5, random_state=42, n_jobs=-1
                          )
...
try:
    ...
    X = pd.DataFrame.sparse.from_spmatrix(
        X, columns=get_ct_feature_names(preprocessor))
